...
def handleTrace(dut, trace, traceIdx, tracingMode):
    ...
    if _CYCLE_STATUS_ENUM is None:
        _CYCLE_STATUS_ENUM = extractSVEnum('../../hw5-pipelined/cycle_status.v', 'cycle_status_e')
        pass
    if tracingMode == 'generate':
        ...
        traceElem['trace_writeback_inst'] = f'0x{dut.datapath.trace_writeback_inst.value.integer:08x}'
        ...
        assertEquals(int(traceElem['trace_writeback_inst'],16), dut.datapath.trace_writeback_inst.value.integer, msg)
